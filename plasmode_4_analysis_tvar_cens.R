...
###############################################################################################
#
# Project: Time-varying plasmode simulation from EAGeR data (with censoring)
#
...
n <- 1226		        # Sample size
n_mc <- 2500        # Size of Monte Carlo resample
...
# Exposure weights
ps <- glm(X ~ XLast + Z1 + Z1Last + Z2 + Z2Last + white + smoke + age + BMI + as.factor(Int), 
...
# Censoring weights
den_d <- glm((D==0) ~ X + Z1 + Z2 + white + smoke + age + BMI + as.factor(Int), 
          family=binomial(link="logit"), data=boot)$fitted.values
...
mod.Y <- flexsurvreg(Surv(Int0, Int, Y) ~ X + XLast + Z1 + Z1Last + Z2 + Z2Last + white + smoke + age + BMI, 
                     data=boot, dist="exp")
...
pgf <- function(ii, mc_data, length, exposure=NULL){
  ...
  gdat <- data.table(id, time, Xp, Z1p, Z2p, white, smoke, age, BMI, Yp, p_y)
  return(gdat)
}
...
long3 <- left_join(long2, select(boot, bid, Int, X, Z1, Z2, Y, D), by=c("bid", "Int")) %>% 
  group_by(bid) %>% 
...
conf_Z1 <- long3 %>% 
  select(bid, Int, Z1) %>% 
  pivot_wider(names_from=Int, names_prefix="Z1", values_from=Z1) 
conf_Z2 <- long3 %>% 
  select(bid, Int, Z2) %>% 
  pivot_wider(names_from=Int, names_prefix="Z2", values_from=Z2) 
...
res <- ltmle(data=select(wide, -bid), 
             Anodes=Anodes, Lnodes=Lnodes, Ynodes=Ynodes, Cnodes=Cnodes,
...
filename <- paste("../results/tvar_cens_n-", n, "_mc-", n_mc, ".csv", sep="")
