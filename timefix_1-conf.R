...
simulation <- function (exposure) {
  ...
  if (is.null(exposure)){
    p_a <- 1/(1 + exp(-(-log(1/0.5 - 1) + 0.5*L - 0.5*0.3)))
    A <- rbinom(n, 1, p_a)
  }
  else {
    A <- exposure
  ...
  p_t  <- (0.05)*exp(log(2)*A + 0.5*L) #In main analysis, lambda=0.01; for common outcome, lambda=0.05
  Tv <- rexp(n, p_t)
  Z <- as.numeric(Tv < 10)
...
cat('\n')
cat("Fitting oracle",'\n')
oracle.aft <- flexsurvreg(Surv(Tv, Z) ~ A + L, data=oracle, dist="exp")
...
logit <- predict(glm(A ~ L, family=binomial(link="logit"), data=boot))
...
## G-computation
    ...
    mod.D <- flexsurvreg(Surv(Tv, Z) ~ A + L, data=boot, dist="exp")
    ...
    pgf<-function(mc_data, exposure){
      ...
      p_t <- exp(coef(mod.D)[names(coef(mod.D))=="rate"])*
        exp(coef(mod.D)[names(coef(mod.D))=="A"]*A + coef(mod.D)[names(coef(mod.D))=="L"]*L)
...
write.table(all.res, file="timefix_1-conf_n-5000_common.txt", sep="\t")
